...
internal class ActiveMqRecipientRegistrar : IRecipientRegistrar
{
	/// <summary>
	...
	/// 当前注册器的日志记录器实例。
	/// </summary>
	private readonly ILogger<ActiveMqRecipientRegistrar> _logger;
	...
	/// <see cref="Apache.NMS.ISession"/> 与消费者。若不保留引用，这些会话将无法释放。
	/// </remarks>
	private readonly List<ActiveMqRecipient> _recipients = [];

	/// <summary>
	/// 初始化 <see cref="ActiveMqRecipientRegistrar"/> 的新实例。
	/// </summary>
	/// <param name="configurator">提供约定与策略解析的消息总线配置器。</param>
	/// <param name="provider">用于创建接收器并解析依赖项的服务提供程序。</param>
	/// <param name="options">包装在 <see cref="IOptions{TOptions}"/> 中的已配置 <see cref="ActiveMqBusOptions"/>。</param>
	/// <param name="logger">用于创建当前注册器类型化日志记录器的日志工厂。</param>
	public ActiveMqRecipientRegistrar(IConfigurator configurator, IServiceProvider provider, IOptions<ActiveMqBusOptions> options, ILoggerFactory logger)
	{
		_convention = configurator.Convention;
		_provider = provider;
		_options = options.Value;
		_strategy = configurator.GetStrategy(_options.Name);
		_logger = logger.CreateLogger<ActiveMqRecipientRegistrar>();
	}
	...
	/// - 根据消息约定解析合适的接收器实现：
	///   单播 -> <c>ActiveMqConsumer</c>，
	///   多播 -> <c>ActiveMqSubscriber</c>，
	///   请求 -> <c>ActiveMqExecutor</c>；
	/// - 在注册的通道上启动接收器。
	/// </summary>
	...
	public async Task RegisterAsync(IDictionary<string, ChannelRegistration> registrations, string defaultTransporter, CancellationToken cancellationToken = default)
	{
		...
		{
			_logger.LogInformation("[ActiveMqRecipientRegistrar] Registering {MessageType} on channel {Channel}", registration.MessageType.FullName, channel);
			...
			ActiveMqRecipient recipient = conventionType switch
			{
