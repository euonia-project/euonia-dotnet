...
/// <summary>
/// 表示 ActiveMQ 消费者，用于从指定的消息通道接收消息。
/// </summary>
internal class ActiveMqConsumer : ActiveMqRecipient
{
	/// <summary>
	/// 初始化一个新的 ActiveMqConsumer 实例。
	/// </summary>
	/// <param name="provider">服务提供者，用于解析依赖项。</param>
	/// <param name="channelName">消息通道的名称。</param>
	/// <param name="messageType">消息的类型。</param>
	public ActiveMqConsumer(IServiceProvider provider, string channelName, Type messageType)
		: base(provider, channelName, messageType)
	{
	}

	/// <summary>
	/// 启动 ActiveMQ 消费者，开始接收消息。
	/// </summary>
