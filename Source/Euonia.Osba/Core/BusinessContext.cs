...
public class BusinessContext
{
	...
	/// <summary>
	/// 获取当前的 <see cref="IObjectFactory"/> 实例。
	/// </summary>
	public IObjectFactory ObjectFactory => field ??= CurrentServiceProvider.GetRequiredService<IObjectFactory>();
	...
	{
		return ObjectFactory.Create<TTarget>(criteria);
	}
	...
	{
		return ObjectFactory.Fetch<TTarget>(criteria);
	}
	...
	/// 每个条件项必须与方法参数类型匹配。
	/// </remarks>
	public Task<TTarget> CreateAsync<TTarget>(params object[] criteria)
	{
		return ObjectFactory.CreateAsync<TTarget>(criteria);
	}
	...
	/// 每个条件项必须与方法参数类型匹配。
	/// </remarks>
	public Task<TTarget> FetchAsync<TTarget>(params object[] criteria)
	{
		return ObjectFactory.FetchAsync<TTarget>(criteria);
	}
}